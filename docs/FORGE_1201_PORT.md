...
* 终极微缩仪式已改为原版工作台配方，并按 1.20.1 实际五芒星树使用锻造 Marid、
  附身 Afrit、召唤 Marid、召唤野生高等魔灵。
...
* `gradlew runClient "-Pmekanism_magic.occultism_runtime=false"` 可在无 Occultism
  运行依赖时完成基础客户端加载。
* 已在实际世界中完成机器放置、容器、GUI、模块槽位、侧面配置、升级、配方处理、
  JEI、仪式数据和维度矿工输出验证。
* 未安装 Mekanism Extras 时不再加载高阶工厂配方或引用其未注册物品。

因此当前分支已达到 Forge 1.20.1 正式版 `1.0.0` 发布条件。
